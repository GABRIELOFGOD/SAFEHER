...
const LoginAdmin = () => {
  ...
  const { username, loginError, loginMsg, adminLogin } = useUserContext()

  return (
    <div className={`absolute duration-300 flex items-center justify-center z-50 bg-primary bg-opacity-80 top-0 left-0 h-[100vh] w-[100vw]`}>
        <div className='p-12 bg-white relative rounded-xl'>
            ...
            {loginError && <p className='w-full border-2 border-red-800 rounded-md text-red-800 bg-red-300 text-center'>{loginError}</p>}
            <form className='flex flex-col gap-5' onSubmit={(e) => adminLogin(e, email, password)}>
                ...
                <button className='bg-primaryDark py-3 px-4 text-white text-[12px] uppercase font-semibold w-full rounded-md'>Login</button>
            </form>
        </div>
    </div>
  )
